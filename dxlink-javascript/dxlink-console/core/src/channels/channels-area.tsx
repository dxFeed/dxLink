 ...
 * (`null` — the default — meaning all of them). The second filters buttons only — an
 * already-open channel keeps rendering, so a profile that disagrees with what is on screen
 * degrades instead of crashing.
 */
export const ChannelsArea = ({ channels }: ChannelsAreaProps) => {
  const config = useConsoleConfig()
  const restricted = config.channelKinds
  const offered =
    restricted === null ? channels : channels.filter((plugin) => restricted.includes(plugin.kind))
