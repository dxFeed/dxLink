...
export interface DXLinkScriptCandlesubscription {
...
const isCandleEvent = (event: DXLinkScriptCandle): event is DXLinkCandleEvent =>
...
export interface DXLinkScriptCandlesOptions {
  /**
...
export class DXLinkScriptCandles {
  private readonly script: DXLinkScript
  ...
  constructor(private readonly client: DXLinkClient) {
    this.script = new DXLinkScript(client)

    // Listen to events
    this.script.addDataListener((events) => {
  ...
  set = (subscription: DXLinkScriptCandlesubscription, script: string) => {
    this.script.set(
      {
        type: 'Candle',
        ...subscription,
