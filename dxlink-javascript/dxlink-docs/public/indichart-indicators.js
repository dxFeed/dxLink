...
function onTick() {
	spline(aroonUp(high, n), {title: "aroonUp", offset: 0, type: spline.STYLE_LINE, color: color.GREEN})
	spline(aroonDown(low, n), {title: "aroonDown", offset: 0, type: spline.STYLE_LINE, color: color.RED})
...
function onTick() {
	const pc = close[1]
	const d1 = Math.abs(high - low)
	const d2 = Math.abs(high - pc)
	const d3 = Math.abs(low - pc)
...
window['INDICHART_INDICATROS'] = {
  "dxscript-js": INDICHART_JS_INDICATORS,
}
